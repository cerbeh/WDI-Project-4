  ...
  render() {
    return <div>
      <section className="hero is-fullheight">
        <div className="split left slide-in-top">
          <div className="centered">
            <h1 className="title text-focus-in">KENDO</h1>
          </div>
        </div>

        <div className="split right slide-in-top-delayed">
          <div className="centered">
            <h1 className="title underline text-focus-in-delayed">KENDO.IO</h1>
            <h2 className="subtitle text-focus-in-more-delayed">
              Focus on being better than you were yesterday
            </h2>
            {!Auth.isAuthenticated() && <Link to="/login"><i className="fas fa-arrow-alt-circle-right fa-5x text-focus-in-more-delayed"></i></Link>}
            {Auth.isAuthenticated() && <Link to={`/users/${Auth.getPayload().sub}`}><i className="fas fa-arrow-alt-circle-right fa-5x text-focus-in-more-delayed"></i></Link>}
          </div>
        </div>
